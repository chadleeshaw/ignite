...
func (h *SyslinuxHandler) DeactivateVersion(w http.ResponseWriter, r *http.Request) {
	...
	for _, v := range versions {
		if v.Version == version && v.Active {
			// Remove boot files
			if err := h.service.RemoveBootFiles(r.Context(), version, "bios"); err != nil {
				http.Error(w, "Failed to remove BIOS boot files: "+err.Error(), http.StatusInternalServerError)
				return
			}
			
			if err := h.service.RemoveBootFiles(r.Context(), version, "efi"); err != nil {
				http.Error(w, "Failed to remove EFI boot files: "+err.Error(), http.StatusInternalServerError)
				return
			}
			...
			// TODO: Save version back to repository
			// For now this is a limitation since we don't have a SaveVersion method in the service interface
